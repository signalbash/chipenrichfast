test_chipenrich_score = function(geneset,gpw,n_cores) {
	...
	results_list = parallel::mclapply(as.list(ls(geneset@set.gene)), function(go_id) {
		single_chipenrich_score(go_id, geneset, gpw, fitspl, 'chipenrich')
	}, mc.cores = n_cores)
...
single_chipenrich_score = function(go_id, geneset, gpw, fitspl, method) {
