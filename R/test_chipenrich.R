test_chipenrich = function(geneset,gpw,n_cores) {
    ...
    results_list = parallel::mclapply(as.list(ls(geneset@set.gene)), function(go_id) {
        single_chipenrich(go_id, geneset, gpw, fitspl, 'chipenrich', model)
    }, mc.cores = n_cores)
...
single_chipenrich = function(go_id, geneset, gpw, fitspl, method, model) {
