...
geneset = do.call(rbind,
				  lapply(1:length(temp), function(index) {
				  	data.frame(gs_id = rep(names(temp)[index], length(temp[[index]])),
															 gene_id = unlist(strsplit(temp[[index]],split = " ")),
															 stringsAsFactors = F)}))
...
Poly-Enrich is strongly recommended when using either the 'enhancer' or 'enhancer_plus5kb' gene locus definition, because only polyenrich is able to properly split the weight of genomic regions that are assigned to multiple genes (multiAssign=TRUE). Like chipenrich, it is designed for narrow peaks, but for experiments with > ~50,000 genomic regions, or in cases where the number of binding sites per gene is thought to be important. 

Poly-Enrich also allows weighting of individual genomic regions based on a score. Currently the options are: `signalValue` and `logsignalValue`. `signalValue` weighs each peak based on the Signal Value given in the narrowPeak format or a user-supplied column, while `logsignalValue` take the log of these values. To use weighted Poly-Enrich, use method = `polyenrich_weighted`. When using the `enhancer` or `enhancer_plus5kb` locus definition, we recommend selecting the `polyenrich_weighted` method, which in this case will automatically set multiAssign as `TRUE`.
...
print(results.prox[1:5,c(1,2,4,5,6,7)])
```

The last column of ProxReg results indicates whether this geneset is farther or closer to the regulation region. In the above example, these five genesets are significantly farther from promoter regions.
