...
Poly-Enrich also allows weighting of individual peaks. Currently the options are: `signalValue` and `logsignalValue`. `signalValue` weighs each peak based on the Signal Value given in the narrowPeak format or a user-supplied column, normalized to have mean 1. `logsignalValue` weighs each peak based on the log Signal Value given in the narrowPeak format or a user-supplied column, normalized to have mean 1. When use weighted Poly-Enrich, `method` should use `polyenrich_weighted`. When selecting `polyenrich_weighted` method and `locusdef` equals to `enhancer` or `enhancer_plus5kb`, the `multiAssign` will be automatically set as `TRUE`.
