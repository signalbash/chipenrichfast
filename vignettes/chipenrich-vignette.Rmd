...
* `hybridenrich()` is used when one is unsure of which method, between ChIP-Enrich or Poly-Enrich, is the optimal method. It will be more conservative than either test for an individual gene set, but is usually more powerful overall. 
...
The proximity to regulatory region (proxReg) test is an addendum to any gene set enrichment test, not just exclusive to the methods in this package. It tests if the peaks tend to bind closer to farther to gene transcription start sites or enhancer regions in the gene sets. Currently, testing to proximity to enhancer regions is only compatible with the hg19 genome. Its purpose is to include further information to an analysis, as a gene set enrichment test alone may not give information about if a region is binding in a promoter or enhancer region.

ProxReg first calculates the distance between the midpoints of peaks to the nearest transcription start site or the nearest enhancer region midpoint for each genomic region. Each genomic region is then assigned to the gene with the nearest transcription start site. The distances are then group based on if the genes are in a the gene set or not, and a signed Wilcoxon rank-sum test is used to calculate if the regions are closer or farther in the gene set than average.
