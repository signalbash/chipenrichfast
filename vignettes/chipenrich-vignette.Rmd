...
```
```{r}
# For a specific organism e.g. hsa:
head(supported_genesets("hsa"))
```

A description of our built-in gene sets:
...
* `biocarta_pathway`: BioCarta Pathway, Ver 6.0. (biocarta.com)
...
* `mesh`: Gene Annotation with MeSH, the National Library of Medicine's controlled vocabulary for biology and medicine. Last updated ~2013. (gene2mesh.ncibi.org)

* `metabolite`: Metabolite (NCIBI) (ncibi.org)
...
C.T. Lee , R.G. Cavalcante, T. Qin, S. Patil, S. Wang, C. Lee, M.A. Sartor. “Poly-Enrich: Extensions to the ChIPEnrich method and updates to tools.” In preparation.
